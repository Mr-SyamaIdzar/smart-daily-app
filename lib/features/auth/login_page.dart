...
class _LoginPageState extends State<LoginPage> {
  ...
  void _onEmailFocusChanged() async {
    if (!_emailFocus.hasFocus && _emailController.text.isNotEmpty && !_hasPromptedBiometric) {
      final provider = context.read<AuthProvider>();
      final hasBio = await provider.hasBiometricForEmail(_emailController.text);
      if (mounted && hasBio) {
  ...
  Widget _buildBiometricButton(AuthProvider provider) {
    return OutlinedButton.icon(
      onPressed: provider.isLoading ? null : () => provider.loginWithBiometric(_emailController.text),
